...
def uber_search(search_terms, DEBUG_LEVEL):
    ...
    if len(list_of_search_terms) > 1: # If more than one iterate but return None
        ...
        for index, search_term in enumerate(list_of_search_terms):
            print(f'uber_search:{DEBUG_LEVEL}')
            df = execute_search(search_term, DEBUG_LEVEL)
        ...
        df = execute_search(list_of_search_terms[0], DEBUG_LEVEL)
